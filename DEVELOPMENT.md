...
Everything behind the app: building it, releasing it, and why it works the way
it does. For installing and using it, see the [README](README.md); for drawing a
charm, see [ASSETS.md](ASSETS.md).
...
`install.sh` clears that flag for you: it takes the latest release, checks it
against the checksum in `Casks/tassel.rb` and copies the app across. One line,
and no Homebrew needed.

```bash
curl -fsSL https://raw.githubusercontent.com/desilva23/Tassel/main/install.sh | bash
```

It is also published through Homebrew. This repository is its own tap: the cask
is `Casks/tassel.rb`. Homebrew 7 requires a non-official tap to be trusted
before it will load, and a tap only resolves from a short `user/name` when the
repository is called `homebrew-name` — so both the trust and the tap have to
name the URL.
...
To publish a new version, from a clean `main`:

```bash
make release VERSION=0.1.3 NOTES="What changed."
```

That runs the checks, bumps the version, builds the disk image, points the cask
at it, releases it on this repository's
[Releases](https://github.com/desilva23/Tassel/releases) page and confirms the
public download matches the build. Anyone who has it already picks the new
version up with `brew upgrade tassel`.
...
maneki neko, the red of its thread on the bronze Yansheng — so they read as
beads, not as bits of the charm. The nazar, the bell and the pysanka keep plain
bone. **Beads on the Rope** turns the threading off.
...
something anyone owns; none of its source, assets, copy, or design was taken. The emoji charms are Unicode
code points drawn with the system emoji font, and the drawn charms are original
work — see [ASSETS.md](ASSETS.md), and keep it current as you add things.
...
To ship a fork under a name of your own, check it is free on the App Store and
as a domain, then:
